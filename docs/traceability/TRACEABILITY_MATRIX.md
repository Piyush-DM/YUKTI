...
| `CHOIR_v0.1_RESEARCH_FREEZE.md` §3 | Frozen commitments, as executable contract | DAALE conformance lock (v0 Phase 0) | `daale/conformance/` | `daale/tests/test_conformance.py` (28) | 28 commitments: 20 conforming, 8 unexercised, 0 unresolved | `daale/README.md` | **First row with a real upstream source.** §3 is accepted frozen content; the register transcribes it and the lock resolves every citation. No `DECISION-005` dependency. |
| `CHOIR_v0.1_RESEARCH_FREEZE.md` §3 (A2, X4) | Canonical control plane and commit authority | DAALE D-Core, Reasoning State Store, trace (v0 Phases 1–2) | `daale/execution/` | `daale/tests/test_execution.py` (37) | Deterministic trace; input-bound execution identity | `daale/README.md` | **Implemented to the `DECISION-005` boundary.** Contract validation transcribes A2; referential integrity applies X4 at the input boundary. No evaluator: that requires the unresolved A/B choice. |
| `CHOIR_v0.1_RESEARCH_FREEZE.md` §3.1, §4 | Why each unexercised commitment is unexercised | Gap classification | `daale/conformance/classification.py` | `TestClassification` (7) | 8 classified across 5 categories | `daale/README.md` | **Classified, not queued.** Four are §4 research programmes. `E12` is preserved as an evidence discrepancy pending adjudication. |
| `DECISION-007` | Candidate escalation: which origins may commit | Commit gate origin routing | `daale/execution/commit.py` (`DETERMINISTIC_ORIGINS`) | `test_a_non_deterministic_origin_escalates_rather_than_commits`, `test_a_deterministic_origin_validates`, `test_an_escalated_candidate_leaves_canonical_state_untouched` | None | `DECISION-007` | **Approved 2026-08-14.** Allow-list, default-deny. Supersedes assumption G3. Adding an origin requires an approved decision. |
| `DAALE_v0_CLOSURE.md` | DAALE v0 closure and architectural freeze | DAALE v0 as a whole | `daale/conformance/`, `daale/execution/` | 77 for `daale/` | Conformance lock; boundary guard | `DAALE_v0_CLOSURE.md` | **CLOSED 2026-08-14.** Ten frozen properties, enforced by existing mechanisms. Five items carried out unresolved, headed by `DECISION-005`. |
